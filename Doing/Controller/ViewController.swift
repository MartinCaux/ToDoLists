...
class ViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "addItem") {
            ...
            destVC.itemToEdit = dataModel.list[indexOfSelectedCell!.row].item
        }
    }
}
...
extension ViewController:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = dataModel.list[indexPath.row].item.name
...
extension ViewController:ItemDetailViewControllerDelegate {
    ...
    func listDetailViewController(_ controller: ItemDetailViewController, didFinishAddingItem item: Item) {
        controller.dismiss(animated: true)
        dataModel.list.append(ItemView(item: item))
        dataModel.sortCheckLists()
        let indexOfNewItem = dataModel.list.index(where: {$0.item === item})
    ...
    func listDetailViewController(_ controller: ItemDetailViewController, didFinishEditingItem item: Item) {
        controller.dismiss(animated: true)
        let indexOfPreviousItem = dataModel.list.index(where: {$0.item === item})
        dataModel.sortCheckLists()
        let indexOfEditedItem = dataModel.list.index(where: {$0.item === item})
        ...
        dataModel.list.insert(ItemView(item: item), at: indexOfEditedItem!)
